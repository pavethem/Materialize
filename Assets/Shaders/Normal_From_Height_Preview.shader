Shader "Custom/Normal_From_Height_Preview"
{
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            {
                float2 UV = IN.uv;
                float4 heightTex = tex2Dlod(_MainTex, float4(UV, 0, 0));
                half4 normalTex = half4(tex2Dlod(_BlurTex0, float4(UV, 0, 0)).xyz, 1.0) * _Blur0Weight;
                ...
                normalTex = normalTex + blurTex1 + blurTex2 + blurTex3 + blurTex4 + blurTex5 + blurTex6;

                normalTex *= 1.0 / normalTex.w;

                normalTex.xyz = normalize(normalTex.xyz * 2.0 - 1.0);

                float3 angularDir = normalize(float3(normalize(float3(normalTex.xy, 0.001)).xy * _AngularIntensity,
                                                     max(1.0 - _AngularIntensity, 0.001)));
                normalTex.xyz = lerp(normalTex.xyz, angularDir, _Angularity);

                normalTex.xy = normalTex.xy * _FinalContrast;
                normalTex.z = pow(saturate(normalTex.z), _FinalContrast);

                normalTex.xyz = normalize(normalTex.xyz) * 0.5 + 0.5;
                ...
                float3 finalColor = lerp(heightTex.xxx, normalTex.xyz,
                                         smoothstep(_Slider - 0.01, _Slider + 0.01, UV.x));
