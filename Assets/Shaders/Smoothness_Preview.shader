        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                if (_UseSample1)
                {
                    ...
                    float sample1HueDif = 1.0 - min(
                        min(abs(blurTexHSL.x - sample1HSL.x), abs((blurTexHSL.x + 1.0) - sample1HSL.x)),
                        abs((blurTexHSL.x - 1.0) - sample1HSL.x)) * 2.0;
                    float sample1SatDif = 1.0 - abs(blurTexHSL.y - sample1HSL.y);
                    float sample1LumDif = 1.0 - abs(blurTexHSL.z - sample1HSL.z);
                    sample1Mask = (sample1HueDif * _HueWeight1) + (sample1SatDif * _SatWeight1) + (sample1LumDif *
                        _LumWeight1);
                ...
                if (_UseSample2)
                {
                    ...
                    float sample2HueDif = 1.0 - min(
                        min(abs(blurTexHSL.x - sample2HSL.x), abs((blurTexHSL.x + 1.0) - sample2HSL.x)),
                        abs((blurTexHSL.x - 1.0) - sample2HSL.x)) * 2.0;
                    float sample2SatDif = 1.0 - abs(blurTexHSL.y - sample2HSL.y);
                    float sample2LumDif = 1.0 - abs(blurTexHSL.z - sample2HSL.z);
                    sample2Mask = (sample2HueDif * _HueWeight2) + (sample2SatDif * _SatWeight2) + (sample2LumDif *
                        _LumWeight2);
                ...
                float3 finalColor = lerp(mainTex.xyz, finalSmoothness.xxx,
